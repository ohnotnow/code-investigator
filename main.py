...
def strip_markdown(text: str) -> str:
    """ strip markdown code blocks from the start and end of the text """
    lines = text.split('\n')
...
if __name__ == "__main__":
    ...
    args.add_argument("--request", type=str, required=False, help="The request to the agent")
    args.add_argument("--mode", type=str, required=False, default="code", help="The mode to run the agent in")
    args.add_argument("--model", type=str, required=False, default="o4-mini", help="The model to use for the agent")
    args.add_argument("--no-readme", action="store_true", required=False, default=False, help="Do not use a GitHub Readme style for the output")
    args.add_argument("--output-file", type=str, required=False, default=None, help="The file to write the output to")
    args.add_argument("--rewrite-output", action="store_true", required=False, default=False, help="Rewrite the output using a more creative LLM model before writing to the output file")
    ...
    if args.rewrite_output:
        ...
        final_output = rewrite_result.final_output
        final_output = strip_markdown(final_output)
    print(final_output)
    if args.output_file:
